...
std::vector<std::pair<int,int> > arrange_words(std::vector<std::pair<int,int> > all_words,int maxBlobHeight){
  ...
  while(rows_assigned==0){
	...
	for(int i=1;i<=row_count;i++){
		int row_entry = 0;
		for(int j=0;j<size;j++){
			if(row_determined[j]==i)row_entry++;
			else continue;
			int greater_than = 0;
...
/*std::vector<std::pair<int,int> > arrange_words(std::vector<std::pair<int,int> > all_words,int maxBlobHeight){
  ...
  std::cout<<"to be arranged words:"<<std::endl;
  for(int i=0;i<size;i++){
    std::cout<<all_words[i].second<<" "<<all_words[i].first<<std::endl;
  }
  std::cout<<std::endl;
  std::vector<std::pair<int,int> > new_words(size);
  for(int i=0;i<size;i++){
    int count=0;
    for(int j=0;j<size;j++){
      if(abs(all_words[i].first-all_words[j].first)>0.67*maxBlobHeight){
        if(all_words[i].first>all_words[j].first){
          count++;
          continue;
        }
      }
      else {
        if(all_words[i].second>all_words[j].second){
          count++;
          continue;
        }
      }
  ...
  std::cout<<"arranged words:"<<std::endl;
  for(int i=0;i<size;i++){
    std::cout<<new_words[i].second<<" "<<new_words[i].first<<std::endl;
  }
  std::cout<<std::endl;
  return new_words;
}
